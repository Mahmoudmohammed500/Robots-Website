...
export default function RobotTrolleyPanel({ 
  ...
  // الحصول على أسماء كل الأزرار النشطة
  ...
  // الحصول على كل الأزرار النشطة للعرض
  ...
  const activeStaticButtons = allButtons.filter(btn => displayButtons.includes(btn));
  const activeCustomButtons = displayButtons.filter(btn => !allButtons.includes(btn));
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            {/* Static Buttons */}
            <div className="mb-4">
              <label className="text-xs text-gray-400 mb-2 block">Static Buttons:</label>
              <div className="flex flex-wrap gap-2">
                {allButtons.map((btn) => {
                  const isActive = displayButtons.includes(btn);
                  return (
                    <button
                      key={btn}
                      type="button"
                      ...
                      className={`px-4 py-2 rounded-xl border transition-colors flex items-center gap-2 ${
                        isActive 
                          ...
                          : "bg-gray-100 text-gray-700 border-gray-300 hover:border-main-color"
                      }`}
                    >
                      {btn} 
                      {isActive && "✓"}
            ...
            {/* Custom Active Buttons */}
            <div className="mb-4">
              <label className="text-xs text-gray-400 mb-2 block">Custom Active Buttons:</label>
              <div className="flex flex-wrap gap-2">
              ...
              {activeCustomButtons.length === 0 && (
                <div className="text-gray-500 italic text-sm">No custom active buttons</div>
              )}
            </div>
          </div>
