      ...
      >
        ...
        >
          ...
          <p className="text-gray-600 mb-6">
            ...
            <span className="font-semibold text-main-color">{buttonName}</span>?
...
export default function ButtonSetting() {
  ...
  useEffect(() => {
    const fetchButton = async () => {
      ...
      try {
        setLoading(true);
        const allButtons = await getData(`${BASE_URL}/buttons`);
        const buttonsArray = Array.isArray(allButtons)
          ? allButtons
          : allButtons?.data || [];
        const foundButton = buttonsArray.find(
          (btn) =>
            String(btn.BtnID ?? btn.id ?? btn.BtnId ?? btn.ID) ===
            String(buttonId)
        );
  ...
  const handleAddButton = async () => {
    try {
      setSubmitting(true);
      const validName =
        buttonName && buttonName !== "undefined" ? buttonName : "Button";

      const createdButton = await postData(`${BASE_URL}/buttons`, {
        BtnName: validName,
      ...
      try {
        const robotData = await getData(`${BASE_URL}/robots/${robotId}`);
        const newButtonObj = {
      ...
      setSubmitting(false);
      navigate(`/homeDashboard/robotSettings/${robotId}`, {
        state: { refresh: true },
      });
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (robotData) {
          ...
          if (
            updatedRobotData.buttons &&
            Array.isArray(updatedRobotData.buttons)
          ) {
            updatedRobotData.buttons = updatedRobotData.buttons.filter(
              (btn) =>
                String(btn.BtnID ?? btn.id ?? btn.BtnId ?? btn.ID) !==
                String(btnId)
            );
            await putData(`${BASE_URL}/robots/${robotId}`, updatedRobotData);
          }
        }
      } catch (updateError) {
