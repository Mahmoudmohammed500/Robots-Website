...
export default function ButtonSetting() {
  ...
  // load colors from localStorae
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem("buttonColors") || "{}");
    setSavedColors(stored);
    if (stored[`${id}-${buttonName}`]) {
      setColor(stored[`${id}-${buttonName}`]);
    }
  }, [id, buttonName]);

  //change color and save to localStorage
  useEffect(() => {
    const updated = { ...savedColors, [`${id}-${buttonName}`]: color };
    setSavedColors(updated);
    localStorage.setItem("buttonColors", JSON.stringify(updated));
  }, [color]);
